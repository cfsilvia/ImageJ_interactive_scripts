...
roiManager("open", inputDir + "Pvn.zip"); 
Rois = CreateArrayIndex(roiManager("count"));
selectWindow('label');
run("Flatten");
run("RGB Color");
...
//Create montage of 2 images
run("Concatenate...", "open image1=[use-1] image2=[label-1]");
rename("Concatenate");
selectWindow("Concatenate");
run("Make Montage...", "columns=2 rows=1 scale=1 label");
saveAs("Tiff", outputDir + rName + "Label" + list[i]);
////////////////////////
   }
...
IJ.renameResults("AllCells","Results");
    saveAs("Results", outputDir +"SummaryCounting.xls");
