...
function FindIndex(	FileListROI,qRoi,BeginROI) {
   for(i = 0; i < qRoi; i++) { 
   	 a = endsWith(FileListROI[i],"_" + BeginROI + ".zip" );
   	 print(FileListROI[i]);
   	 //print(a);
   	 //print(BeginROI);
   	// waitForUser;
...
Dialog.create(title);
Dialog.addChoice("Type:", newArray("Dapi", "Cfos", "Cherry", "Oxytocin","Vasopresin","TH","Fibers","Overlay 2 channels","Overlay 3 channels","Overlay 4 channels","Others")); //this is the name of the folder
...
if(AddROI){
	...
	Dialog.create(" ");
	Dialog.addNumber("From which ROI number to begin -give last number:", 1);
	Dialog.show();
...
//Find the index begin of the initial Roi
 ...
 //////////////////////////  	
   	...
   	Num1 =newArray(qRoi);
   	   for(j = 0; j < qRoi; j++) { 
     a1 = substring(FileListROI[j], 0, lastIndexOf(FileListROI[j], "."));
     IndexAux = lastIndexOf(a1, "_");
     num1 = substring(a1, IndexAux+1);
...
///////////////////////////////////////////////////     
    ...
    if(AddROI){ //open the wanted ROI
     ...
     print(stringy + "ROI/" + New[p] + ".zip");
      roiManager("save", stringy + "ROI/" + New[p] + ".zip");//save roi selection
    // close("Composite");
     //close("Gfp");
     //close("Dapi");
   // close(substring(StringyList[p], 0, indexOf(StringyList[p], "."))+ "overlay.tiff");
    // roiManager("delete");
   close(FileList[p]);
